...
A **Mission Atlas** is a semantic map of many ThunderLang missions. It lets a team
understand a large change without opening every `.thunder` file.
...
scale. The Atlas is how ThunderLang scales beyond one file.
...
| `thunder atlas ./intent [--search q \| --expand id]` | shipped | the navigable/searchable whole-system Atlas |
| `thunder graph ./intent --view atlas` | planned | the rendered visual Atlas view |
| `thunder chains ./intent` | planned | detected mission chains |

`thunder index` is real today. It reports only what is derivable from the `.thunder`
...
slug) and `intentProofHash` (the same sha256 the `.thunder-proof.json` carries as its
...
- ThunderLang (this repo) teaches the concept and the examples.
