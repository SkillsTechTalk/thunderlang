...
```js
import {
  parseIntent, buildIntentGraph,        // .thunder source -> AST -> intent-graph-v1
  ...
  graphToSource,                        // graph -> editable .thunder (round-trip)
...
- **AI provider calls** go through **SkillsTech Runtime (STRU)**, never an SDK in this package.
  `thunder draft` builds a provider-neutral brief; STRU executes it.
...
- **OpenThunder** (Node / TypeScript) , `npm i @skillstech/thunderlang`. Import `parseIntent`,
  ...
  `.thunder` parsing or graph re-derivation.
- **Repo Mastery** (web / TypeScript) , import from `/core`. It can now parse `.thunder` directly
...
- **SkillsTech Mobile** (React Native / Hermes) , import from `/core`. The compiler is now
  ...
  test + steps) is in
  [`examples/adoption/skillstech-mobile/`](https://github.com/SkillsTech/thunderlang-site/tree/main/examples/adoption/skillstech-mobile)
