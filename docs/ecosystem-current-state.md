...
is already built, where concepts are duplicated, and, most importantly, **what ThunderLang
...
| --- | --- | --- | --- |
| **ThunderLang** (this repo) | The language + shared schema | `intent-graph-v1`, `intent-ir-v1` (superset IR), Fable rules (`intent-fable-v1`), the Scanner spine (`intent-scan-v1`), Ledger, Guardian, Simulator, Atlas, Lift, Draft, the AI-implementation contract | Built |
...
ThunderLang's `intent-ir-v1` (`compiler/src/intent-ir.mjs`) is a strict superset of the
...
4. **Two ThunderLang compilers** , this repo's `.mjs` implementation (upstream owner) and
   `SkillsTech/compiler` (a TypeScript `@skillstech/thunderlang` that ST vendors). Divergent
   siblings, not a file-identical fork.

## Boundaries this repo will respect

- **ThunderLang does not build a learning / mastery engine.** RepoMastery owns the
  *deterministic* comprehension/mastery engine and already has the Atlas Learning Compiler;
  ...
  ThunderLang's job is to emit a faithful Intent IR (`thunder scan --ir`, or `/core` directly)
  that RepoMastery projects into lessons and Skills Tech Talk turns into practice.
- **ThunderLang does not add AI SDKs or a provider router.** SkillsTech Runtime (STRU) is the
  provider-neutral gateway with BYOK and no-silent-fallback. ThunderLang's AI surface stays
  provider-neutral (`thunder draft` builds a brief; STRU executes it).
- **ThunderLang owns the schema and the deterministic Scanner.** Filling scanner gaps
  (`thunder risks / gaps / unverified / coverage / unknowns / contradictions`) and keeping the
  IR faithful (e.g. never-rule verification edges) is squarely in charter.

## Shared identifiers and deep links

- `missionId` (slug) + `intentProofHash` (= `.thunder-proof.json` `sourceHash`) are the ratified
...
- Which repo becomes the single home of the ThunderLang compiler (this `.mjs` vs ST's TS)?
