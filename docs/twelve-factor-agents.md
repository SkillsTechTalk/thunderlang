...
IntentLang scores an intent against the 13 principles of
...
points"* , is IntentLang's thesis. So most of the 12 factors map directly onto structure IL
...
```
intent twelve-factor examples/TwelveFactorAgent.intent          # scored report
intent twelve-factor examples/TwelveFactorAgent.intent --json   # machine-readable
```

Programmatic (`@skillstech/intentlang` / `/core`):

```js
import { parseIntent, twelveFactorReport, twelveFactorSummary } from '@skillstech/intentlang/core';
const report = twelveFactorReport(parseIntent(source));   // per-factor verdicts + score
// compileSource(...) and the .intent-proof.json both carry the compact `twelveFactor` summary.
```
...
(`intent explain IL-12F-08`).

## The mapping: each factor → the IntentLang signal the lens inspects
...
## Why IntentLang is a natural fit
...
same shape as an IntentLang **decision table** (FIRST-hit rules + default, fully traced),
...
`examples/TwelveFactorAgent.intent` is a triage agent that scores **100/100** , it declares typed
