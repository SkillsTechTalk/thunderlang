...
Language docs and example `.thunder` files live here under `docs/` and `examples/`
...
- [x] Add `examples/CreateInvoice.thunder`
- [x] Add `examples/ResetPassword.thunder`
- [x] Add `examples/BillingService.thunder`
- [x] Add `examples/InvoiceCreated.thunder`
- [x] Add `docs/spec.md`
...
- [x] Align `.thunder-proof.json` schema to v0.1.0 (schemaVersion, neverRules, proofStatus)
...
- [x] Add `examples/DuplicateInvoicePrevention.thunder`
...
- [x] Semantic-diagnostics catalog documented + CI-guarded: docs/diagnostics.md generated
      from DIAGNOSTIC_RULES (49 rules), `thunder rules`, `thunder explain <CODE>`. Security
...
- [x] Define `implementation-plan.json` and the CST → AST stages in detail
      ...
      its fixed deterministic category order, verified against `thunder build` output.)
...
- [x] Remaining page-by-page prose polish (vision/docs/examples still say "Intent")
      ...
      overclaiming commands (`thunder plan`, `thunder generate` code-gen, `thunder docs`)
      ...
      metadata. tsc clean.
- [x] Surface `docs/` and `examples/` on the site (render `.thunder` + Markdown)
...
- [x] Document Prompt-to-Intent, intent review, missing-guarantee/risk/test
      ...
      deterministic half (`thunder draft` rigor + review checklist, the diagnostics that
      flag missing guarantees/risks/tests, the implementation-plan, `thunder explain` +
      ThunderLens notes) from the optional AI half, and cross-links ai-implementations.
...
- [x] **Proof artifact** - compiler-contract (§8) + manifesto (proof chain); real `.thunder-proof.json`
...
- [x] **OpenThunder** proves the repo matches the declared intent (drift) , IL SIDE DONE:
      `thunder handoff` emits `il-to-ot-drift-v1`; the `@openthunder/intent` consumer +
...
- [~] **Repo Mastery** proves the human understands the mission , IL SIDE READY: missions,
      ThunderLens notes/lenses, and the `@skillstechtalk/thunderlang/core` browser subpath are
...
- [x] **12-Factor Agents conformance lens , SHIPPED (0.1.6).** Deterministic `twelveFactorReport`
      ...
      verdict + score + fix; `IL-12F-01..13` findings; `thunder twelve-factor` CLI; `twelveFactor`
...
Deterministic `@thunderlang/compiler` in `compiler/` (no AI). `thunder check|graph|proof|build`.
Emits `.thunder/<mission>/{contract-graph.json, architecture-graph.json, implementation-plan.json, .thunder-proof.json}`
...
Tested against `examples/CreateInvoice.thunder`.
...
6. Multi-file: compile a whole `*.thunder` tree into one `contract-graph.json` (missions[]).
...
## ThunderLens Notes (compiled semantic comments)
...
Next slices:
- [x] `thunder notes <file> --json` , SHIPPED: lists the compiled `note <lens>:` blocks
...
- [x] `thunder docs --lens <lens>` , SHIPPED: `renderLensDoc(ast, lens)` (pure, browser-safe)
      ...
      verification" disclaimer. `thunder docs <file> [--lens <lens>] [--out <dir>]`. +4 tests.
...
- [x] Completion provider (compiler core): block keywords, note lenses, semantic types,
      context-aware; `thunder completions --position --json`; consumed via /api/assist
- [x] Hover provider (compiler core): semantic types + note lenses; `thunder hover --position --json`
- [x] Code-action provider with safety levels + text edits; `thunder code-actions`, `thunder apply-fix`
      SHIPPED: `getCodeActions(source, diagnostics)` returns safety-graded actions (safe
      autocorrects + reviewable diagnostic quick-fixes). `thunder code-actions <file>` lists
      them; `thunder apply-fix <file> [--write]` applies the safe ones, leaves reviewable
...
- [ ] `thunder format`; later `thunder lsp` (LSP) for VS Code
...
CodeFactsIR -> Inference Engine -> LiftedIntent -> .thunder draft.

Shipped (P0, compiler core + playground, no AI):
- [x] `compiler/src/lift.mjs`: CodeFactsIR + TypeScript adapter (functions,
      params, return types, tests, errors) + inference + humble .thunder renderer
...
- [x] CLI `thunder lift --from typescript <file> [--json] [--out dir]`
...
Next slices:
- [x] `thunder lift --from repo <path>` (walk files, one mission/file, unique names,
...
- [x] `thunder approve` (reviewed:true + source_hash + reviewer/time; recognized block)
- [x] `thunder drift <code> --intent <file>`: re-lift + compare (guarantee/never/input
      unsupported, stale proof, new behavior); playground lift->approve->drift demo
- [x] `thunder handoff` emits `il-to-ot-drift-v1` (mission, approval+hash, mapsTo,
...
## Mission Atlas , scaling beyond one .thunder file

> Teaching model for navigating MANY missions: one file is not enough when Claude
> Code / Codex or a team generate dozens or hundreds of `.thunder` files a day. This
...
- [x] `examples/mvp-customer-portal/` , worked example: valid `.thunder` missions across
...
- [x] Implemented `thunder index <dir> [--json]` for real: `compiler/src/atlas.mjs`
      (`buildMissionIndex`) + CLI wiring + 3 vitest-style node tests (23/23 pass). Reports
      only .thunder-derivable fields (mission, area, risk heuristic, guarantee/never counts,
...
- [x] `examples/mvp-customer-portal/mission-index.json` is now a GENERATED golden fixture
      (real `thunder index --json` output), not hand-authored.
- [x] Un-marked `thunder index` from planned -> shipped in the docs (mission-atlas,
...
- [ ] Republish `@skillstechtalk/thunderlang` (bump) so npm users get `thunder index`.
- [ ] `thunder chains` (needs a chain-declaration convention or reliable type-linkage detection).
- [ ] `thunder summarize` / `diff` / `release` (need git history + test/drift evidence).
...
- [x] Add stable join keys to `mission-index-v1` , `missionId` (deterministic mission slug) +
      `intentProofHash` (byte-identical to the `.thunder-proof.json` sourceHash). Additive; golden
...
- [x] Add `sourceProduct: "skillstech-compiler"` to `.thunder-proof.json` (asked by STCE, for cert-proof keying).
      Additive envelope field (after schemaVersion); sourceHash/join keys unchanged; +assertion. 23/23 tests.
- [x] Add an ESM library entry to `@skillstechtalk/thunderlang` (asked by ST, #134500): `src/index.mjs` curated
...
- [x] Parse an `errors:` block (PascalCase failure names) (ST #220000): parser + AST + `.thunder-proof.json`
...
- [ ] Build `thunder compile --target openthunder` (native `.openthunder/missions/<id>/contract.yaml`,
...
- [ ] `thunder index ./intent` , mission inventory (mission-index.json)
- [ ] `thunder graph ./intent --view atlas` , Mission Atlas view
- [ ] `thunder chains ./intent` , detect + render mission chains
- [ ] `thunder summarize ./intent --since today` , Build Session Digest
- [ ] `thunder diff ./intent --since HEAD~1` , Semantic Diff
- [ ] `thunder proof matrix ./intent` , Proof Matrix
- [ ] `thunder release ./intent --mvp` , MVP Readiness Report + Release Story
...
- [x] `compiler/src/ai.mjs`: 9-state model + `blocksProduction`; shared multi-language managed-region
      ...
      (normalized); `.thunder/ai-implementations.json` manifest (`buildManifest`); provider-neutral
...
- [x] CLI: `thunder ai list`, `thunder ai generate`. Library barrel + `index.d.ts` export the AI API.
- [x] Docs `docs/ai-implementations.md`; example `examples/CalculateRiskScore.thunder`. 34/34 tests.
...
- [ ] Phase 2 OT: verification pipeline (region integrity/syntax/types/contract/effects/architecture/
      security/tests/determinism), proof at `.thunder/proofs/{id}.json`, SARIF (INTENT-AI-* rules),
...
- [x] IL follow-ups (SHIPPED): state resolver (`resolveState`: declaration + region + proof -> real state,
      with stale-proof MODIFIED on hash mismatch), `productionGate` + `thunder ai gate [--allow-pending]`,
      `thunder build --mode production` gate wiring, `thunder ai adopt` (marker rewrite AI->human, provenance
      preserved). Exported from the library. 38/38 tests.
- [x] IL follow-ups (SHIPPED): `thunder ai approve/reject` + `.thunder/ai-approvals.json` store , decisions
...
- [x] intent-ai-v1 reconcile (from OT #216 + RM redline): extracted pure helpers to `ai-core.mjs`
      (zero Node deps) + published `@skillstechtalk/thunderlang/core` subpath so RM's browser bundle can import
...
- [x] Deterministic candidate selection (SHIPPED): `compiler/src/select.mjs` , `selection` block
      ...
      require-checks filter, stable tiebreak). `thunder ai select <dir> <id>`. An LLM never picks. Pure
...
- [x] IL follow-ups (SHIPPED): an events sink/log. `compiler/src/ai-events.mjs`
      ...
      serializeEventLog/timeline, rejects unknown types). `thunder ai approve|reject` now
      persist their event to `.thunder/ai-events.jsonl`; `thunder ai events [dir]
...
- First-demo wedge: `thunder build` emits docs, Mermaid, test plan, contract/architecture graphs, `.thunder-proof.json` (no AI)
...
- [x] `bin` wiring + npm publish (#40,#94): published `@skillstechtalk/thunderlang@0.1.0`
      (public, MIT). `npm i -g @skillstechtalk/thunderlang` -> `intent`; `npx @skillstechtalk/thunderlang`.
...
- [x] `thunder check` in a GitHub Actions workflow (#95): scripts/intent-check.mjs
      batch-runs the real CLI over every authored .thunder (skips the .thunder/ output
...
- [x] Canonical Intent Graph (`compiler/src/intent-graph.mjs`, `buildIntentGraph`): typed
      nodes + relationships, deterministic; emitted as `intent-graph.json` by `thunder build`.
...
- [x] Example `examples/CertificationStudyPlan.thunder`; docs/intent-graph.md; library +
      index.d.ts exports; 56/56 tests. Backward-compatible (existing .thunder files unaffected).
...
- [x] Experience Contract syntax (experience/actor/goal/enter-when/journey/state/responsive/
      ...
      blocker, role-rendered) + IL-EXP-001. Example UploadStudyMaterial.thunder; 59/59 tests.
...
- [x] Constraint composition + conflict resolution , the reconciliation layer (the ecosystem
      ...
      Intent Graph. Example CertificationCheckout.thunder. 66/66 tests. An LLM never resolves.
...
- [x] Canonical schema (non-negotiable #1, anti-fork) , `compiler/src/intent-schema.mjs`: canonical
      ...
      DIAGNOSTIC_RULES catalog (stable IDs). `thunder schema` emits it. Test enforces buildIntentGraph can
      ...
      canonical source + `thunder schema` output serve as the binding source now.)
- [x] Gap 2 temporal/lifecycle , SHIPPED: `lifecycle` state machine (state/transition[from/to/within]/
      ...
      CertificationAttempt.thunder. 73/73 tests. OT verifies the implemented reality vs this IR.
- [x] Gap 3 distributed/failure , SHIPPED: `command` (idempotency_key/timeout/retry/backoff), `event`
      ...
      undeclared-event (error). Command/FailureHandler nodes. Example CreateStudyPlan.thunder. 78/78 tests.
- [x] Gap 4 decisions/rules , SHIPPED: `decision` (inputs/rule[when/return/priority]/default/explanation/
      ...
      CertificationEligibility.thunder. 84/84 tests. OT verifies rule coverage + impl match.
- [x] Intent Atlas (directive #4) SHIPPED: buildAtlas/searchAtlas/expandNode over the graph; `thunder atlas` (overview/search/expand); whole-system map over 26 example missions (132 nodes). Mission Atlas = mission-index-v1.
- [x] Semantic diff (directive #4) SHIPPED: diffGraphs + `thunder diff <before> <after>` , added/removed/changed nodes + edges + invalidated-approvals-on-contract-change (deterministic). 94 tests.
- [x] Semantic MERGE (directive #4, the pair) SHIPPED: mergeGraphs(base,ours,theirs) + `thunder merge <base> <ours> <theirs>` , deterministic 3-way merge by node identity/meaning, structured conflicts (keeps ours), presence-based edge merge, exit 1 on conflict (CI). Typed IntentMerge/`intent-merge-v1`. 99 tests. Substrate for ST collaborative Atlas editing. cfa380f.
- [x] Gap 5 governance/waivers SHIPPED: `waiver <CODE>` block (reason/approved_by/scope/expires) parsed to ast.waivers; governance.mjs applyWaivers + governanceDiagnostics (IL-GOV-001..005), schema `intent-governance-v1`; wired into `thunder check` (waived blockers shown on-record, don't fail build; `--now` for deterministic expiry). Exported + typed. 107 tests. docs/governance.md.
...
- [x] OUTCOME CONTRACTS SHIPPED (last language item): `outcome_contract <Name>` (outcome/metric/baseline/target/direction/window/owner) -> OutcomeContract node (added to NODE_TYPES) + Mission-requires + targets Outcome + measured_by Metric (resolved). EXECUTABLE: outcome.mjs evaluateOutcomeContract(contract, actual) (direction-aware higher/lower, improvement vs baseline) + evaluateOutcomes(ast) auto-matches delivery `result` -> met/missed/pending. `thunder outcomes <file>` (exit 1 on missed, CI-ready). Authoring checks IL-OC-001..004 (no target/metric/window, target-not-better-than-baseline) via semanticDiagnostics (gate-safe). schema `intent-outcome-v1`. Exported + typed. 191 tests. docs/outcome-contracts.md. Closes the intent arc: outcome -> release -> result -> CONTRACT VERDICT.
