...
// The thin, deterministic data layer that turns one `.intent` source into what a mobile
...
//
// Requires: `@skillstech/intentlang` installed. React Native 0.81 / Expo 54 resolve the
...
/**
 * Turn one `.intent` source into the data a mobile lens screen renders. Deterministic:
...
export function missionLens(source: string, depth = 2): MissionLens {
  ...
  const scan: ScanResult = scanProject([{ file: 'mobile.intent', source }]);
